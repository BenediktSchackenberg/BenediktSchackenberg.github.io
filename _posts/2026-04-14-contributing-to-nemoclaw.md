...
The core idea is simple but important: when you give an AI agent access to your terminal, your files, and your code, you need guardrails. Not because AI is inherently malicious, but because AI is *confidently wrong* in ways that can be catastrophic. A tool that can write to your filesystem and run shell commands can also accidentally (or not so accidentally) overwrite your SSH keys.

NemoClaw solves this by:

1. **Sandboxing** the agent inside an OpenShell container with Landlock-based filesystem isolation
2. **Network policies** that define exactly which hosts the agent can reach (and how)
3. **Defense-in-depth** — multiple independent layers so no single bypass compromises everything
4. **A pretty good onboarding flow** — `nemoclaw onboard` and you're running in minutes
